    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #eef2ff 0%, #f8fafc 100%)' }}>
      <div style={{ width: 380, background: '#fff', border: '1px solid #eee', borderRadius: 12, padding: 24, boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>登录你的账号以管理与创建行程</div>
        </div>
        <form onSubmit={submit}>
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            <div>
              ...
              <input value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" type="password" required style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e7eb', borderRadius: 8 }} />
            </div>
            <button type="submit" disabled={loading} style={{ marginTop: 4, width: '100%', background: '#111827', color: '#fff', border: 0, borderRadius: 8, padding: '10px 12px', cursor: 'pointer' }}>{loading ? '登录中…' : '登录'}</button>
            {error ? <div style={{ color: '#ef4444', fontSize: 13 }}>{error}</div> : null}
          </div>
        </form>
        <div style={{ marginTop: 12, fontSize: 13, color: '#6b7280', textAlign: 'center' }}>
          还没有账号？ <Link href="/register" style={{ color: '#2563eb', textDecoration: 'none' }}>去注册</Link>
        </div>
      </div>
    </div>
  );
